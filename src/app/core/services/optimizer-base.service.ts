...
import {
  BlockedCraft, SubCraftItem, ScanGroup,
  collectSubCrafts, collectBlockedCrafts, wouldCraft, resolveIngredientsCost, buildScanGroups,
} from './xp-optimizer.utils';

export type { BlockedCraft, SubCraftItem, ScanGroup };

export abstract class BaseOptimizerService<TRow extends XpRecipe> {
  ...
  readonly dialogRow            = signal<TRow | null>(null);
  readonly dialogQty            = signal<number>(1);
  readonly dialogCheckedBlocked = signal<Set<number>>(new Set());

  abstract readonly rows: Signal<TRow[]>;

  protected abstract getLevelRange(playerLevel: number): { min: number; max: number };
  ...
  protected async loadCategoryRecipes(categoryId: number, playerLevel: number): Promise<void> {
    const { min, max } = this.getLevelRange(playerLevel);
    const recipes = await window.electronAPI.getRecipesByCategoryAndLevel(categoryId, min, max);
